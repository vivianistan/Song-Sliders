...
LOOP
	LDR R1, =SSI0_SR_R
	LDR R2, [R1]						;1) read and check bit 4
	AND R2, R2, #0x10
	CMP R2, #0							;2) if high, loop back to 1 (BUSY)
	BNE	LOOP
	
	LDR R2, =DC							;3) clear Data Command pin (PA6) to zero (CHECK)
	LDR R3, [R2]
	MOV R4, #0xFFFFFFBF
	AND R3, R3, R4						;Clear bit 6
	STR R3, [R2]					
	
	LDR R1, =SSI0_DR_R
	STR R0, [R1]						;4) write command to SSI0_SR_R
	
LOOP2
	LDR R1, =SSI0_SR_R
	LDR R2, [R1]						;5) read and check bit 4
	AND R2, #0x10
	CMP R2, #0
	BNE LOOP2							;6) if high, loop back to 5   (BUSY)  
...
BACK
	LDR R1, =SSI0_SR_R
	LDR R2, [R1]
	AND R2, #0x02					;1) Read and check bit 1
	CMP R2, #0x02
	BNE BACK						;2) Loop if Low (not ready)
	
	MOV R2, #0x00000040
	LDR R3, =DC
	LDR R4, [R3]					;Get PA6?
	ORR R2, R4, R2					;3)Set to PA6 one
	STR R2, [R3]					
	
	LDR R1, =SSI0_DR_R
	STR R0, [R1]					;4)write to SSI0_DR_R
